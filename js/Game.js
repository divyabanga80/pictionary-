class Game {
  ...
  async updateWords() {
    ...
    if (!words.exists() && !words.val()) {
      database.ref('/').update({
        words: ['cow', 'hen', 'house', 'snake', 'clown', 'island', 'wall', 'chair', 'tree', 'triangle','tower','stars','giraffe','rhino','Baby Flower','Rainbow', 'Beard'
       ,'Flying saucer','Camera','	   Lamp' ]
      })
    }
  }
  ...
  async newGuess() {
    ...
    if (newWord && newWord === drawingWord) {
  ...
  play() {
    ...
    if (player && allPlayers !== undefined) {
      ...
      if (isDrawing && !(mouseX < colorWheel.width && mouseY < colorWheel.height)) {
        var point = {
          x: mouseX,
          y: mouseY,
          color: lineColor
